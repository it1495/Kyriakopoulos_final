...
public class FetchMerchantsTask extends AsyncTask<String,Void,ArrayList<Merchant>> {
    ...
    private ArrayList<Merchant> getMerchantsFromJson(String merchantJsonStr) throws JSONException {
        ...
        try{
            ...
            for(int i = 0; i < merchantsArray.length(); i++) {
                Merchant EMPORAS = new Merchant();

                JSONObject currentMerchant = merchantsArray.getJSONObject(i);

                EMPORAS.setId(Integer.toString(currentMerchant.getInt("id")));
                EMPORAS.setLegalName(currentMerchant.getString("name"));

                JSONObject Ratings = currentMerchant.getJSONObject("aggregateRating");
                String rating = Integer.toString(Ratings.getInt("ratingValue"));

                EMPORAS.setReview(rating);

                JSONObject Katigoria  = currentMerchant.getJSONObject("merchantCategory");

                EMPORAS.setCategory(Katigoria.getString("name"));

                JSONObject POSnartho = currentMerchant.getJSONObject("contactPoint");

                EMPORAS.setAddress(POSnartho.getString("streetAddress"));

                merchants.add(EMPORAS);
            }
    ...
    protected void onPostExecute(ArrayList<Merchant> merchants) {
        if(merchants.size() > 0){
            ...
            for (Merchant EMPORAS: merchants){
                merchantAdapter.add(EMPORAS);
            }
        }
    }
}
